...
if (args.includes("--help") || args.includes("-h")) {
  console.log("用法：node prepare-redskill.mjs --source <本地文件夹|ZIP|链接> --output <输出目录> [--skill <相对路径>] [--display-name <展示名称>] [--skill-id <英文内部 ID>] [--short-intro <简介>] [--version <版本>] [--source-type <原创|转载>] [--permissions <权限建议>]\n\n创建独立 REDSkill 候选副本和提交资料草案，绝不修改源文件；仅在没有阻断项时生成上传 ZIP。");
  process.exit(0);
}
...
function writeDocs(skillFile) {
  ...
  const sourceType = value("--source-type") || "待确认：原创或转载";
  const permissions = value("--permissions") || "默认建议：不需要权限；若实际代码访问相册、摄像头、麦克风或本地存储，请按真实行为选择并说明用途。";
  ...
  walk(outputDir, (file) => {
    ...
    totalBytes += bytes;
    if (bytes > 10 * 1024 * 1024) blockers.push({ kind: "BLOCKER_SINGLE_FILE_SIZE", target: relative(outputDir, file), excerpt: "官方上传页显示单文件最大 10MB。" });
  });
  if (totalBytes > 30 * 1024 * 1024) blockers.push({ kind: "BLOCKER_TOTAL_SIZE", target: "上传源码", excerpt: "官方上传页显示总大小不超过 30MB。" });
  if (totalBytes > 10 * 1024 * 1024) warnings.push({ kind: "WARN_ZIP_SIZE", target: "上传源码", excerpt: "候选目录总大小已超过 10MB；最终 ZIP 可能超过单文件限制，请生成后再次核对。" });
  ...
  const submission = {
    ...
    forge_suggestions: {
      ...
      version,
      permissions,
      source_and_attribution: sourceType
    }
  };
  writeFileSync(join(outputDir, "REDSKILL-SUBMISSION-FIELDS.md"), `# REDSkill 上传资料建议\n\n本文件把官方页面已确认的要求与 Forge 建议分开记录。第二步实际出现哪些输入框，以用户账户的实时页面为准。\n\n## 一、官方上传页第一步\n\n上传文件：\`${submission.official_upload_step.upload_source_zip}\`\n\n- [ ] 最外层直接有 \`SKILL.md\`。\n- [ ] 单文件不超过 10MB，全部源码总大小不超过 30MB。\n- [ ] 内容来源：${submission.official_upload_step.source_type}\n- [ ] 已阅读 \`REDSKILL-AUDIT.md\`，且没有阻断项。\n- [ ] 已阅读并同意《Skill 发布安全规范》。\n\n## 二、Forge 填写建议\n\n### 展示名称\n\n${submission.forge_suggestions.display_name}\n\n### 英文内部 ID\n\n\`${submission.forge_suggestions.internal_id}\`\n\n> 此 ID 用于包名和作者管理，不声称它一定是官方页面字段。\n\n### 简介\n\n${submission.forge_suggestions.intro}\n\n### 版本号\n\n\`${submission.forge_suggestions.version}\`\n\n### 来源与授权\n\n${submission.forge_suggestions.source_and_attribution}\n\n### 权限建议\n\n${submission.forge_suggestions.permissions}\n\n## 提交前最后核对\n\n- [ ] 页面、文档和代码描述的是同一项能力。\n- [ ] 没有承诺“必过审核”、官方背书或源码未实现的效果。\n- [ ] 实时页面与本文件不一致时，按实时页面修改，并同步更新本文件。\n`);
...
try {
  ...
  if (blockers.length) {
    ...
    if (statSync(zipPath).size > 10 * 1024 * 1024) {
      rmSync(zipPath);
      writeFileSync(join(outputDir, "REDSKILL-AUDIT.md"), "\n## ZIP 大小阻断\n\n- 最终 ZIP 超过官方页面当前显示的 10MB 单文件上限，因此未保留上传 ZIP。请缩减非必需资源后重新运行。\n", { flag: "a" });
      console.error(`最终 ZIP 超过 10MB，已停止交付上传包。审计报告：${join(outputDir, "REDSKILL-AUDIT.md")}`);
      process.exitCode = 1;
    } else {
